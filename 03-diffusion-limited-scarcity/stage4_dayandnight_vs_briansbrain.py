...
OTHER_BIRTH_SET, OTHER_SURVIVE_SET = {3, 6, 7, 8}, {3, 4, 6, 7, 8}  # Day and Night
...
def propose_next(state, rng, other_birth=OTHER_BIRTH_SET, other_survive=OTHER_SURVIVE_SET):
    """'GOL' variable names are retained for continuity with the earlier
    GoL-vs-Brian's-Brain harness, but the birth/survive rule for that
    species is now a parameter -- other_birth/other_survive default to
    Day and Night's rule (birth={3,6,7,8}, survive={3,4,6,7,8}) for this
    pairing. Brian's Brain's logic is unchanged."""
    gol_mask = state == GOL
    ...
    # "GOL"-slot species (here: Day and Night): survives per its own rule,
    # blind to BB
    gol_survives = gol_mask & np.isin(gol_neighbor_count, list(other_survive))
    proposed[gol_survives] = GOL
    # death (gol_mask & ~gol_survives) -> proposed stays OFF (default)
    ...
    # OFF cell resolution: birth eligibility per each rule, blind to each other
    gol_birth_eligible = off_mask & np.isin(gol_neighbor_count, list(other_birth))
...
def landauer_gated_step(state, energy, D_bg, D_diff, rng, bb_cost_multiplier=1.0,
                          flip_cost=FLIP_COST, ceiling=ENERGY_CEILING):
    """bb_cost_multiplier (alpha): Brian's Brain pays alpha * flip_cost for
    ALL its transitions (ignition, Firing->Refractory, Refractory->OFF).
    GOL always pays the unmodified flip_cost. This isolates whether BB's
    territorial dominance survives as its own metabolic burden rises,
    holding GOL's established baseline behavior fixed."""
    ...
    # per-cell cost: alpha*flip_cost for any transition INTO or WITHIN the
    # BB lifecycle (ignition from OFF, or BB's own mandatory transitions);
    # unmodified flip_cost otherwise (GOL death, GOL birth)
    is_bb_transition = (state == BB_FIRING) | (state == BB_REFRACTORY) | \
                        ((state == OFF) & (proposed == BB_FIRING))
    ...
    energy_next = np.where(actually_flips, energy - cost_grid, energy)
...
def run_condition(D_bg, D_diff, seed=0, bb_cost_multiplier=1.0, warmup=WARMUP_STEPS, window=WINDOW_STEPS, n_mi_pairs=150):
    # Both ceiling AND warmup must scale with bb_cost_multiplier. Ceiling:
    # otherwise BB's transitions become categorically unaffordable above a
    # fixed cap, regardless of alpha's specific value (confirmed directly:
    # alpha=20 through 200 gave bit-for-bit identical frozen results at a
    # fixed ceiling of 10.0). Warmup: reaching the first affordable
    # transition takes roughly alpha/D_bg steps of pure accumulation: a
    # fixed warmup that is fair for low alpha systematically starves high
    # alpha of the time needed to even begin, producing a MISLEADING
    # "frozen" result that looks like a real dynamical outcome but is
    # actually just "hasn't started yet" (confirmed directly: alpha=20
    # showed zero activity at step 200-300, but a thriving population of
    # 2000+ by step 800 given adequate warmup).
    ...
    for step in range(effective_warmup):
        state, energy, _ = landauer_gated_step(state, energy, D_bg, D_diff, rng, bb_cost_multiplier=bb_cost_multiplier, ceiling=dynamic_ceiling)
    ...
    for t in range(window):
        ...
        n_bb = int(((state == BB_FIRING) | (state == BB_REFRACTORY)).sum())
        if n_gol == 0 and extinction_step['GOL'] is None:
            extinction_step['GOL'] = effective_warmup + t
        if n_bb == 0 and extinction_step['BB'] is None:
            extinction_step['BB'] = effective_warmup + t
