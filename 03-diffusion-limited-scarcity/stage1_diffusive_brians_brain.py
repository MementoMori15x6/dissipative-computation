"""
Brian's Brain on a diffusing energy field.

Section 2 gave every cell an independent, unconditional energy income --
...
This replaces that with a genuine shared, spatially-limited resource:
  ...
  2. Real diffusion: energy flows from higher-energy cells to lower-
     ...
     inflow term.
  3. The same Landauer gate as Section 2: a cell can only execute its
...
Section 2's mechanism: a cell's neighbors' activity can now directly
affect whether IT can afford to flip, since spending drains a shared
local pool that diffusion only slowly replenishes. The three things
being tested, impossible to even pose under a flat-income model:
