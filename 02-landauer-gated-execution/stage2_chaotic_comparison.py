...
and gliders, per the validated attractor-selection finding) and a genuinely
...
RULES = {
    ...
    # Out-of-sample tests of the Section 3.2 criterion. Classified by
    # inspection of the transition table BEFORE measurement:
    #   Seeds: nothing survives, so no live cell can hold its state ->
    #          no non-trivial absorbing configuration -> demand PINNED.
    #   LifeWithoutDeath: every live cell survives, so a live cell's
    #          proposed state equals its current one -> absorbing
    #          configurations abundant -> demand SUPPLY-DEPENDENT.
