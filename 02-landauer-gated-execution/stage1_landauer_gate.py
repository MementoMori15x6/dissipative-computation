...
Every prior test in this line let the rule execute unconditionally and used
...
transition can happen AT ALL. Each step:
  ...
  3. A cell can only execute its proposed flip if it has enough energy
     ...
     there. This is the real on/off condition for rule execution itself
     that no previous test in this line had.
...
TWO MEASURED QUANTITIES, over a trailing window after warmup:
  ...
  C -- complexity: mean time-lagged mutual information between live
    ...
    validated binary MI estimator from the discrete-substrate causal-insulation work.
