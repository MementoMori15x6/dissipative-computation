...
def landauer_gated_step(state, energy, D_bg, D_diff, rng, K, bb_cost_multiplier=1.0,
                          flip_cost=FLIP_COST, ceiling=ENERGY_CEILING,
                          energy_on_capture_transfer=ENERGY_ON_CAPTURE_TRANSFER):
    """bb_cost_multiplier (alpha): extends the Section 4.2 cost-multiplier
    convention to displacement competition. Any transition involving a
    Brian's Brain state -- as current state (mandatory cycling, or being
    captured by Game of Life) or as proposed state (ignition from OFF,
    or capturing an occupied Game-of-Life cell) -- costs alpha times the
    base flip cost. This is the natural extension of Section 4.2's
    definition (there, only current-state-BB or OFF-to-BB-ignition
    existed; capture adds two new transition types, both folded into
    the same "any BB-involving transition" cost rule)."""
    ...
    # Energy accounting: every executed flip pays cost_grid out of its
...
def run_condition(D_bg, D_diff, K, seed=0, bb_cost_multiplier=1.0, warmup=WARMUP_STEPS, window=WINDOW_STEPS):
    # Same two fixes as Section 4.2's cost sweep, required again here:
    # ceiling must scale with bb_cost_multiplier (a fixed ceiling makes
    # high alpha categorically unaffordable regardless of its specific
    # value), and warmup must scale with bb_cost_multiplier / D_bg
    # (reaching the first affordable transition takes time proportional
    # to cost divided by income rate).
    ...
    for _ in range(window):
        ...
        state, energy, actually_flips, capture_direction = landauer_gated_step(
            state, energy, D_bg, D_diff, rng, K, bb_cost_multiplier=bb_cost_multiplier, ceiling=dynamic_ceiling)
