...
def run_condition(D_bg, D_diff, seed=0, warmup=WARMUP_STEPS, window=WINDOW_STEPS, n_mi_pairs=150, grid_n=N):
    rng = np.random.default_rng(seed)
    state = np.full((grid_n, grid_n), OFF, dtype=int)
    ...
    trajectory_bb = np.zeros((window, grid_n, grid_n), dtype=int)
    ...
    for _ in range(n_mi_pairs):
        r_, c_ = rng_mi.integers(0, grid_n), rng_mi.integers(0, grid_n)
        dr, dc = rng_mi.choice([-1, 0, 1]), rng_mi.choice([-1, 0, 1])
        if dr == 0 and dc == 0:
            continue
        r2, c2 = (r_ + dr) % grid_n, (c_ + dc) % grid_n
