...
OTHER_BIRTH_SET, OTHER_SURVIVE_SET = {3, 6, 7, 8}, {3, 4, 6, 7, 8}  # Day and Night; "GOL" variable names retained for continuity
...
def propose_next_with_capture(state, rng, K, bb_pressure_states=BB_PRESSURE_STATES,
    ...
    gol_survives = gol_mask & np.isin(gol_neighbor_count, list(OTHER_SURVIVE_SET))
    ...
    gol_birth_eligible = off_mask & np.isin(gol_neighbor_count, list(OTHER_BIRTH_SET))
