...
convergence-based runner for Game of Life vs. HighLife (Section 4.4's
disambiguation pairing, lottery competition, D_bg=0.05, D_diff=0.05).

No cost-multiplier dimension (symmetric FLIP_COST). Note this module's
landauer_gated_step returns FOUR values (state, energy, actually_flips,
b6_births_executed), one more than the D&N/BB-family modules -- this
tripped an initial unpacking bug, caught immediately by the traceback
before any bad data was produced.
"""
...
sys.path.insert(0, ".")
import stage_gol_vs_highlife as m
...
def run_to_convergence(D_bg, D_diff, seed, warmup=200, check_interval=1000,
    ...
    for _ in range(warmup):
        state, energy, _, _ = m.landauer_gated_step(state, energy, D_bg, D_diff, rng)
...
def sweep(D_bg, D_diff, n_seeds, verbose=True, **kwargs):
    ...
    sd_share = float(np.std(shares)) if shares else None
    if verbose:
        print(f"D_bg={D_bg}, D_diff={D_diff}: GoL-total {a_total}/{n_seeds}, HighLife-total {b_total}/{n_seeds}, "
              f"mean GoL share {mean_share:.4f} (sd {sd_share:.4f}), not-converged {len(not_converged)}/{n_seeds} "
              f"{not_converged if not_converged else ''}, max steps {max_steps}")
    return {'a_total': a_total, 'b_total': b_total, 'n_seeds': n_seeds, 'mean_a_share': mean_share,
            'sd_a_share': sd_share, 'not_converged_seeds': not_converged, 'max_steps_seen': max_steps}


if __name__ == "__main__":
    print("GoL vs HighLife, D_bg=0.05, D_diff=0.05, 15 seeds:")
